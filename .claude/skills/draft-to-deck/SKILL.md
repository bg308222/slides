...
這幾條不是風格偏好，是「這份簡報做完了」的定義。任何一條沒過就還沒交付，
不管內容編排得多好。每次驗證都要逐條確認；日後發現新的問題型態，就往這份清單加。

1. **箭頭與線條一律用「長出來」的動畫，不准憑空出現。**
   一條線直接蹦出來，觀眾得先花時間辨認它從哪指到哪；沿路徑畫出來的話，
   方向本身就是動畫的一部分，不必再用嘴巴補。做法是 `stroke-dasharray` 設成線長、
   `stroke-dashoffset` 從全長跑到 0，箭頭頭再用短延遲的 pop 補在最後（見上一節）。
   同一份簡報裡若有多條箭頭，動畫語彙必須一致 —— 不能有的用畫的、有的直接跳出來。

2. **沒有任何元素可以跑出它該待的框。**
   判準不只是「沒掉出投影片邊界」：表格撐破外框、文字溢出容器、SVG 圖元超出 viewBox，
   全部都算。最常見的成因是外框寫死高度，而裡面的內容（多一列表格、多一行說明）長高了，
   剛好貼齊或超過框線。
   這類問題有一半程式化掃描抓不到 —— 兩個元素都還在投影片內，只是彼此重疊 ——
   所以掃描之外一定要看截圖，特別是「一個外框包住多個子元素」的頁面。

3. **除非那個東西本來就該動，否則相鄰兩幀之間不准有任何位移。**
   典型情況：某一列從純文字換成有 border 與 padding 的 pill，容器高度跟著變，
   同一列的標籤就往下跳幾 px。觀眾會看到「抖一下」，而且注意力會被那個抖動吸走，
   剛好錯過這一格真正要看的東西。
   預防方式是給會變高的那一列固定 `height` 並用 `items-center`，或先把最終尺寸的空間預留好。
   刻意的移動不在此限（例如某個物件從左格滑到右格），但要確認**只有該動的那一個在動**。

驗證第 3 條要用量的，不能靠眼睛：對同一頁逐格量同一個元素的座標，數字必須完全一致。

```js
// browser_evaluate：抖動檢測。所有回傳值必須一模一樣
async () => {
  const nav = window.__slidev__.nav, sleep = ms => new Promise(r => setTimeout(r, ms));
  const PAGE = 10, LABEL = 'session', MAX = 5;      // 改成要檢查的頁、元素文字、click 上限
  const out = {};
  for (let c = 0; c <= MAX; c++) {
    nav.go(PAGE, c); await sleep(320);
    const pg = document.querySelector('.slidev-page-' + PAGE);
    const cb = pg.querySelector('.relative[style*="880px"]').getBoundingClientRect();
    const k = cb.width / 880;                        // 換算回元件自己的座標系
    let el = null;
    pg.querySelectorAll('span,div').forEach(e => {
      if (!e.children.length && e.textContent.trim() === LABEL) el = e;
    });
    out['c' + c] = el ? +(((el.getBoundingClientRect().top - cb.top) / k)).toFixed(2) : null;
  }
...
同時逐條對照上一節的「畫面品質底線」，那份清單就是這一步的檢查表。

**動畫太快，靜態截圖看不到 —— 要凍結它才算真的看過。** 一個 0.3 秒的箭頭動畫，
等你發出截圖指令時早就跑完了，截到的是結束狀態，等於沒驗到。
做法是先移除再重新加上那個 class（中間 `void el.offsetWidth` 強制 reflow）讓動畫重跑，
或注入 `animation-delay: -0.16s; animation-play-state: paused` 把它凍在中間那一格，
然後才截圖。回報時要說清楚你看到的是凍結的中間格，不是最終畫面。
...
| 某個 click 什麼也沒發生 | frontmatter 的 `clicks: N` 開多了，最後幾個是空轉。實際對照元件的 step 上限。反過來也會：`clicks` 開太少，寫了 `v-click="3"` 的收尾句永遠不會出現。 |
| 表格／清單的最後一列壓在外框線上 | 外框寫死高度，剛好等於內容堆疊的總高，縮放後兩條邊重疊。把外框加高十幾 px，別讓它「剛好貼齊」。程式化掃描抓不到（兩者都在投影片內）。 |
| 某一列在某個 click 往下跳幾 px | 那一列的內容從純文字換成帶 border／padding 的元素，容器被撐高。給那一列固定 `height` 並用 `items-center`。 |
...
- **「畫面品質底線」逐條的結果**：箭頭動畫是否都是畫出來的、有沒有元素跑出它的框、
  有沒有量過抖動。抖動那條要附上實際量到的座標，不要只說「沒問題」。
...
- **跳過「畫面品質底線」的逐條確認**——那份清單是交付定義，不是加分項
